...
function askWit(senderID, messageData) {
    request({
        ...
        if (!error && response.statusCode == 200) {
            ...
            // detectCity(senderID, body, messageData);
        }
    });
}

function detectCity(senderID, body, messageData) {
    var tokens = messageData.split(" ");
    console.log(tokens);
    var newArrFound = quickRepliesArray.filter(function(item) {
        for (var i = 0; i < tokens.length; i++) {
            if (item.title.toLowerCase() == tokens[i].toLowerCase()) {
                if (usersMap.get(senderID).search('cityId') !== -1) {
                    if (item.payload === usersMap.get(senderID).get('cityId')) {
                        //dont change cityId
                        console.log("i will not change city id");
                    } else {
                        // change city ID
                        console.log("i will change city Id");
                        searchForEmptyGeneralQuery(senderID, item.payload, messageData, "", "", "");
                    }
                } else {
                    // change city ID
                    console.log("i will change city Id");
        ...
        switch (intentType.entities.intent[0].value) {
            ...
            case 'looking':
                ...
                if (intentType.entities.hasOwnProperty('city')) {
                    var fetchedCity = intentType.entities.city[0].value;
                    console.log("Fetched City " + fetchedCity);
                    if (usersMap.get(senderID).search('cityId') !== -1) {
                        console.log("current city id : " + usersMap.get(senderID).get('cityId'));
                        var newArrFound = quickRepliesArray.filter(function(item) {
                            if (item.title.toLowerCase() === fetchedCity.toLowerCase()) {
                                console.log("FOUND " + item.title);
                                console.log(item.payload == usersMap.get(senderID).get('cityId'));
                                if (item.payload == usersMap.get(senderID).get('cityId')) {
            ...
            case 'interested':
                sendLeadForm(senderID, function(data) {
...
function quickReplies() {
    rootRef.child('quick_replies').on("value", function(snapshot) {
        ...
        // detectCity("121", "aur bra ki haal ae , sub changa in");
...
function searchForPayload(senderID, message, messagePayload) {
    ...
    var arrayFound = quickRepliesArray.filter(function(item) {
        if (item.title == message) {
            ...
            if (item.payload.hasOwnProperty('projectMaxPrice') && item.payload.hasOwnProperty('projectMinPrice')) {
            ...
            } else if (!item.payload.hasOwnProperty('projectMaxPrice') && !item.payload.hasOwnProperty('projectMinPrice') && item.payload !== "other" && !item.payload.hasOwnProperty('bhkCount') && !item.payload.hasOwnProperty('type')) {
                ...
                sendTypingOn(senderID, function(data) {
                    var j = schedule.scheduleJob('*/6 * * * * *', function() {
                        sendUnitTypeButtons(senderID, function(data) {
            ...
            } else if (item.payload.hasOwnProperty('type')) {
                ...
                statusMap.set('type', "" + item.payload.type);
                usersMap.set(senderID, statusMap);
                // //
                console.log('==============================================');
                console.log("type Id : " + usersMap.get(senderID).get("type"));
                console.log('==============================================');
                searchForGeneralQuery(senderID, usersMap.get(senderID).get('cityId'), "", usersMap.get(senderID).get('projectMaxPrice'), usersMap.get(senderID).get('projectMinPrice'), usersMap.get(senderID).get('bhkCount'), usersMap.get(senderID).get('type'));
...
function searchForGeneralQuery(senderID, cityId, queryMessage, maxPrice, minPrice, bhkCount, unitType) {
    console.log("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
...
function searchForEmptyGeneralQuery(senderID, cityId, queryMessage, maxPrice, minPrice, bhkCount, unitType) {
    // console.log("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");
    // console.log("Searching with following params");
    // console.log("Search : " + queryMessage);
...
function sendLeadForm(senderID, callback) {
    var messageData = {
        recipient: {
            id: senderID
        },
        message: {
            attachment: {
                ...
                payload: {
                    template_type: "button",
                    text: "Allright! please fill the form, we will help you taking next steps",
                    buttons: [{
                        type: "web_url",
                        url: "http://www.squareyards.com/contactus?key=help",
                        title: "Fill form"
...
// SEND BHK buttons
function sendBHKButtons(recipientId, callback) {
...
// SEND BHK buttons
function sendUnitTypeButtons(recipientId, callback) {
    console.log('Sending BHK buttons to ' + recipientId);
    var messageData = {
        recipient: {
            id: recipientId
